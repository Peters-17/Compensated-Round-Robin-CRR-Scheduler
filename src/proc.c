...
int checkFull() {
  if((front == 0 && rear == NPROC - 1) || front == rear + 1){
    return 1;
  }
  return 0;
}
...
struct proc* deQueue() {
  ...
  if (checkEmpty()) {
    ...
    element = queue[front % NPROC];
    if (front == rear) {
      front = -1;
      rear = -1;
    } 
    //queue has only one element, reset the queue after dequeing it.
    else {
      front = (front + 1) % NPROC;
    }
    return (element);
...
struct proc* peek() {
  ...
  return queue[front % NPROC];
}

struct proc* dequeueTarget(int pid) {
  struct proc* element = 0;
  if(checkEmpty()) {
    return 0;
  }
  else {
    for(int i = front; i <= rear; i++) {
      if((element = queue[i % NPROC]) -> pid != pid) {
        return 0;
      }
      else {
        if(front == rear) {
...
found:
    ...
    *(uint *)sp = (uint)trapret;
    ...
    p->package.compticks = 0;
    p->package.schedticks = 0;
    p->package.sleepticks = 0;
    p->package.switches = 0;
    ...
    for (;;) {
        ...
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
            ...
            if (p->state == ZOMBIE) {
                ...
                dequeueTarget(pid);  // remove it from the scheduler queue too
...
int CRRHelper(struct proc **external_p) {
    ...
    for (int index = rear; index >= 0; index--) {
        ...
        if (p->state == RUNNABLE) {
            *external_p = p; 
            p->package.switches++;
            return p->timeslice + p->compticks;
        }
    }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    runTime = 0;
    //my work
    while((p && p->state == RUNNABLE && --runTime > 0) || 
                        (runTime = CRRHelper(&p)) != -1){
      if(runTime <= p->compticks) {
        p->compticks -= 1;
        p->package.compticks += 1;
      }
      c->proc = p;
      switchuvm(p);
      p->state = RUNNING;
      //swtich process
      int tempslice = p->timeslice;
      swtch(&(c->scheduler), p->context);
      p->package.schedticks += 1;
      if(tempslice != p->timeslice) {
        runTime = runTime + p->timeslice - tempslice;
      }
      switchkvm();
...
void sleep(void *chan, struct spinlock *lk) {
    ...
    sched();
    //my work
    p->compticks = p->sleepTime;
    p->package.sleepticks += p->sleepTime;
    p->sleepTime = 0;
...
static void wakeup1(void *chan) {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if (p->state == SLEEPING && p->chan == chan &&
            ticks >= (p->timeBeginSleep + p->sleepTime)) {
            p->state = RUNNABLE;
            p->timeBeginSleep = 0;
        }
    }
...
int getpinfo(struct pstat* pstat){
  ...
  if(pstat == 0) {
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      ...
      pstat->compticks[index] = p->package.compticks;
      pstat->schedticks[index] = p->package.schedticks;
      pstat->sleepticks[index] = p->package.sleepticks;
      pstat->switches[index] = p->package.switches;
      index += 1;
    }
    return 0;
  }
